...
output_file = 'task1d_pca.out.txt'
...
def main():
	...
	pca.fit(movie_matrix)
	pca_movie_df = pd.DataFrame(pca.transform(movie_matrix), index=movie_matrix.index)

	input_movie_vector = pd.DataFrame(pca_movie_df.loc[movieid])
	#print(input_movie_vector)


	df_components = pd.DataFrame(pca.components_, columns=actor_matrix.columns).transpose()
