...
output_file = 'task1d_svd.out.txt'
...
def main():
	...
	svd_movie_df = pd.DataFrame(svd.transform(movie_matrix), index=movie_matrix.index)

	input_movie_vector = pd.DataFrame(svd_movie_df.loc[movieid])
	#print(input_movie_vector)


	df_components = pd.DataFrame(svd.components_, columns=actor_matrix.columns).transpose()
