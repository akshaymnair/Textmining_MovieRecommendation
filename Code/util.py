...
def get_tf_idf_matrix(genre):
	...
	# Dataframe with tagid, movieid, sum(timestamp)
	tagid_movieid_grouped = mltags_of_genre.groupby(['tagid', 'movieid'], as_index=False).agg({'timestamp': 'sum'})

	# Dataframe with tagid, movieid, sum(timestamp), t_count
	merged1 = pd.merge(tagid_movieid_grouped, tags_per_movie, on=['movieid','movieid'], how='inner')

	# Dataframe with tagid, movieid, sum(timestamp), t_count, m_count
	merged2 = pd.merge(merged1, movies_per_tag, on=['tagid', 'tagid'], how = 'inner')

	tf_idf_fn = lambda row: (row['timestamp']/row['t_count'])*log(genre_movie_count/row['m_count'])

	result = merged2.apply(tf_idf_fn)
	print result
